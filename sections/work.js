...
function generateWorkExperience() {
  ...
  workExperience.forEach(job => {
    ...
    companyLink.className = 'text-white hover:text-white/80 underline font-medium text-lg block mb-1';
    companyLink.textContent = job.company;
    companyLink.target = '_blank';
    companyLink.rel = 'noopener noreferrer';
    companyLink.setAttribute('aria-label', `View ${job.company} company profile`);
    const roleDuration = document.createElement('p');
    roleDuration.className = 'text-white/60 text-sm sm:text-base';
