const projects = [
  ...
  },
  {
    title: "Medblocks",
    description:
      "Patient management system but using pglite with sync across tabs",
...
function generateProjects() {
  ...
  projects.forEach((project, index) => {
    ...
    iconsContainer.setAttribute(
      "aria-label",
      `Links for ${project.title} project`
    );
    ...
    if (project.deployedUrl) {
      ...
      deployedLink.setAttribute(
        "aria-label",
        `Visit live ${project.title} project`
      );
    ...
    projectDescription.className = "text-sm sm:text-base mb-3";
